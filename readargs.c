...
struct RDArgs * ReadArgs(STRPTR template, IPTR * array, struct RDArgs *rdargs)
/*  FUNCTION
...
#define REST         0x04           /* /F */
    ...
    else
    {
		...
		{
			/* Our args are unescaped! We need to escape them so readargs can
			 parse them the right way. We quote anything that has spaces in
			 it. */
			if (strchr(argv_save[i], ' '))
			{
				char* opt = strtok(argv_save[i], "=");
				char* val = strtok(NULL, "=");
				if (val) {
					/* This is a 'OPT=VAL with spaces'
					Translate to 'OPT="VAL with spaces"' */
					strcat(argbuff, opt);
					strcat(argbuff, "=\"");
					strcat(argbuff, val);
					strcat(argbuff, "\" ");
				} else {
					/* This is a 'VAL with spaces'. Add quotes around it. */
					strcat(argbuff, "\"");
					strcat(argbuff, argv_save[i]);
					strcat(argbuff, "\" ");
				}
			} else {
				/* No space, no escaping needed */
				strcat(argbuff, argv_save[i]);
				strcat(argbuff, " ");
			}
		}
        ...
        lcs.CS_Length = (char*)cs1 - (char*)lcs.CS_Buffer;
        lcs.CS_CurChr = 0;
    ...
    for (arg = numargs; arg-- > 0;)
    {
            ...
            && !(flags[arg] & MULTIPLE))
        {
            if (flags[arg] & KEYWORD)
            {
                ...
                ERROR(B_BAD_INDEX); /* yes, strange error number,
